...
final class Base64 {
	...
	private static byte[] decode(byte[] source, int off, int len, byte[] decodabet) throws Base64DecoderException {
		Object outBuff = new Object[(((len * 3) / 4) + 2)];
		int outBuffPosn = 0;
		byte[] b4 = new byte[4];
				...
				if (sbiDecode >= (byte) -1) {
					if (sbiCrop == (byte) 61) {
						if (paddingByteSeen) {
							...
							if (lastByte == 61 || lastByte == (byte) 10) {
								b4Posn_2 = b4Posn;
							} else {
		...
		Object out = new Object[outBuffPosn];
	...
	private static String encode(byte[] source, int off, int len, byte[] alphabet, boolean doPadding) {
		byte[] outBuff = encode(source, off, len, alphabet, 2147483647);
	...
	private static byte[] encode3to4(byte[] source, int srcOffset, int numSigBytes, byte[] destination, int destOffset, byte[] alphabet) {
		int r3i;
		int r1i = 0;
		r3i = (numSigBytes > 0) ? (source[srcOffset] << 24) >>> 8 : 0;
		int r2i = ((numSigBytes > 1) ? (source[srcOffset + 1] << 24) >>> 16 : 0) | r3i;
		if (numSigBytes > 2) {
			r1i = (source[srcOffset + 2] << 24) >>> 24;
		}
		int inBuff = r2i | r1i;
		switch(numSigBytes) {
