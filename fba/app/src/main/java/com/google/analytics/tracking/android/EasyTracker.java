...
public class EasyTracker extends Tracker {
	...
	private EasyTracker(Context ctx, ParameterLoader parameterLoader, GoogleAnalytics ga, ServiceManager serviceManager, TrackerHandler handler) {
		String r0_String = EASY_TRACKER_NAME;
		if (handler != null) {
			super(r0_String, null, handler);
	...
	private void loadParameters() {
		...
		r10z = this.mParameterFetcher.getBoolean("ga_autoActivityTracking") || this.mParameterFetcher.getBoolean("ga_auto_activity_tracking");
		this.mIsAutoActivityTracking = r10z;
	...
	boolean checkForNewSession() {
		if (this.mSessionTimeout != 0) {
			if (this.mSessionTimeout <= 0 || this.mClock.currentTimeMillis() <= this.mLastOnStopTime + this.mSessionTimeout) {
				return false;
			}
		}
		return true;
